...
import { launchUi, runControlKindMeitnerUi } from "./testing/control-kind-meitner-ui.ts";
...
export class ControlKindMeitnerError extends Error {
...
export const HELP_UI = `renderer (needs a ui launch handle; every verb takes --ui HANDLE, never discovery):
  node --experimental-strip-types scripts/control-kind-meitner.ts ui launch [--entry threads] [--tool-calls JSON] [--mode happy]
...
export const HELP = `control-kind-meitner — verify a running kind-meitner instance through its shared MCP core
...
mutating (an explicit --url or KIND_MEITNER_URL/KIND_MEITNER_PORT is required):
...
isolated fixture:
  node --experimental-strip-types scripts/control-kind-meitner.ts launch
...
export function parse(
  ...
  try {
    ...
    throw new ControlKindMeitnerError(
      ...
      `run control-kind-meitner help for the ${command} syntax`,
    );
  }
}

function required(value: unknown, name: string): string {
  if (typeof value !== "string" || !value.trim()) throw new ControlKindMeitnerError(`${name} is required`);
  return value.trim();
}
...
function positiveInteger(value: unknown, name: string, fallback: number, maximum: number): number {
  ...
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > maximum) {
    throw new ControlKindMeitnerError(`${name} must be an integer from 1 to ${maximum}`);
  }
  return parsed;
}
...
function configuredUrl(raw: unknown, env: NodeJS.ProcessEnv, requiredForMutation: boolean): string | undefined {
  const explicit = typeof raw === "string" && raw.trim()
    ? raw.trim()
    : env.KIND_MEITNER_URL?.trim() || (env.KIND_MEITNER_PORT ? `http://127.0.0.1:${env.KIND_MEITNER_PORT}` : "");
  if (!explicit) {
    if (requiredForMutation) {
      throw new ControlKindMeitnerError(
        "mutating commands require an explicit kind-meitner instance",
        "start `control-kind-meitner launch`, then pass its URL with --url",
      );
    }
    return undefined;
  }
...
function target(values: Record<string, unknown>): { type: "bot" | "channel"; id: string } {
  ...
  if (Boolean(bot) === Boolean(channel)) {
    throw new ControlKindMeitnerError("provide exactly one of --bot ID or --channel ID");
  }
...
export async function runControlKindMeitner(
  argv: string[],
  dependencies: ControlOmbDependencies = {},
): Promise<unknown> {
  const [command = "help", ...args] = argv;
  if (command === "help" || command === "--help" || command === "-h") return HELP;
  if (command === "launch") throw new ControlKindMeitnerError("launch is available only from the executable CLI");
  // The first positional after `ui` is the verb; its tail is parsed strictly per verb.
  if (command === "ui") return runControlKindMeitnerUi(args);
  ...
  if (command === "new-channel") {
    ...
    if (!memberIds.length || new Set(memberIds).size !== memberIds.length) {
      throw new ControlKindMeitnerError("--members must contain unique comma-separated bot IDs");
    }
  ...
  if (command === "send" || command === "send-channel") {
    ...
    if (destination.type !== expected) throw new ControlKindMeitnerError(`${command} requires --${expected} ID`);
  ...
  throw new ControlKindMeitnerError(`unknown command ${JSON.stringify(command)}`, "run control-kind-meitner help");
}
...
export async function launchVerificationServer(
  ...
  if (boxFixtureApi) {
    if (!/^http:\/\/127\.0\.0\.1:[1-9]\d{0,4}$/.test(boxFixtureApi)) {
      throw new ControlKindMeitnerError("Box verification requires an explicit loopback HTTP provider");
    }
    try { new URL(boxFixtureApi); }
    catch { throw new ControlKindMeitnerError("Box verification requires a valid loopback port"); }
  }
  if (localVm) {
    const endpoint = new URL(localVm.host);
    if (endpoint.protocol !== "ssh:" || endpoint.hostname !== "127.0.0.1" || endpoint.password) {
      throw new ControlKindMeitnerError("Local VM verification requires an explicit loopback Podman machine");
    }
  }
  const port = await freePortBlock([0, 1]);
  if (signal?.aborted) throw new ControlKindMeitnerError("verification launch cancelled");
  ...
  const dataDir = mkdtempSync(join(browser && process.platform !== "win32" ? "/tmp" : tmpdir(), "kind-meitner-verify-data-"));
  ...
  const evidenceDir = join(tmpdir(), "kind-meitner-verification-evidence");
  ...
  Object.assign(childEnv, {
    ...
    KIND_MEITNER_PORT: String(port),
    KIND_MEITNER_WEBHOOK_PORT: String(port + 1),
  ...
  if (localVm) Object.assign(childEnv, {
    KIND_MEITNER_EXTRA_PATH: [localVm.binDir, ...(process.platform === "win32" ? [join(childEnv.SYSTEMROOT || "C:\\Windows", "System32")] : [])].join(delimiter),
    CONTAINER_HOST: localVm.host,
    CONTAINER_SSHKEY: localVm.sshKey,
    KIND_MEITNER_STATIC_DIR: localVm.staticDir,
  });
  if (enterprise) Object.assign(childEnv, { KIND_MEITNER_ENTERPRISE_DIR: enterprise.dir, KIND_MEITNER_LICENSE_KEY: enterprise.licenseKey });
  if (browser) Object.assign(childEnv, {
    KIND_MEITNER_AGENT_BROWSER_PATH: browser.binaryPath,
    AGENT_BROWSER_EXECUTABLE_PATH: browser.executablePath,
  });
  if (boxFixtureApi) childEnv.KIND_MEITNER_BOX_API = boxFixtureApi;
  ...
  try {
    for (;;) {
      ...
      try {
        ...
        if (body?.app === "kind-meitner") break;
      } catch {
...
function requireForegroundTerminal(command: string): void {
  if (process.env.npm_lifecycle_event === "control:kind-meitner") {
    throw new ControlKindMeitnerError(
      `${command} must own the terminal directly so Ctrl-C can clean up its children`,
      `run \`node --experimental-strip-types scripts/control-kind-meitner.ts ${command}\``,
    );
  }
}
...
async function main() {
  ...
  const result = await runControlKindMeitner(process.argv.slice(2));
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => {
    const failure = error instanceof ControlKindMeitnerError

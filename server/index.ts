// kind-meitner server — the harness host. Clients hold no transports
...
import { kindMeitnerStatusSystemPrompt } from "./kind-meitner-status-capsule.ts";
import {
...
const STATIC_DIR = process.env.KIND_MEITNER_STATIC_DIR || null;
...
// dir, which must not find a freshly created ~/.kind-meitner already there.
...
const DESKTOP_MANAGED = process.env.KIND_MEITNER_DESKTOP_PARENT === "1";
...
const FALLBACK_PUBLIC_URL = process.env.KIND_MEITNER_PUBLIC_URL?.trim().replace(/\/+$/, "") || null;
...
type DesktopPrivateMessage = BrowserCleanupWireRequest | {
  type: "kind-meitner:browser-control";
  botId: string;
  held: true;
} | {
  type: "kind-meitner:phone-secret-save";
  requestId: string;
  target: string;
  value: string;
} | {
...
function applyDesktopMutationTokenMessage(raw: unknown): boolean {
  ...
  if (message.type !== "kind-meitner:desktop-mutation-token") return false;
...
const browserCleanup: BrowserCleanupCoordinator = new BrowserCleanupCoordinator({
  ...
  send: (request) => {
    ...
    const sessions = request.type === "kind-meitner:browser-bot-deleted" && request.botId
    ...
    const work = status.kind === "ready" && sessions.length
      ? Promise.all(sessions.map(async (session) => {
          ...
          if (!ok) console.warn(`browser cleanup: could not clear saved state for session ${session}; restart kind-meitner to retry this profile's cleanup. Do not use state clear --all: it erases other profiles too.`);
          return ok;
        }))
    ...
    void work.then((results) => results.every(Boolean), (error) => {
      ...
      browserCleanup.receive({ type: "kind-meitner:browser-lifecycle-result", requestId: request.requestId, ok });
    }).catch((error) => {
...
function agentsIntegration(
  ...
  return {
    ...
    env: {
      ...AGENTS_NODE_FLAG,
      KIND_MEITNER_HARNESS_URL: `http://127.0.0.1:${PORT}`,
      KIND_MEITNER_BOT_ID: botId,
      KIND_MEITNER_THREAD_ID: threadId,
      KIND_MEITNER_COMMS_TOKEN: token,
      KIND_MEITNER_TURN_DEPTH: String(depth),
      KIND_MEITNER_ROOM_TURN: roomCoordination ? "1" : "0",
      KIND_MEITNER_OWN_THREAD_CREATION: ownThreadCreation ? "1" : "0",
      KIND_MEITNER_SKILL_AUTHORING_ENABLED: skillAuthoring ? "1" : "0",
      // The shared-computer tools are advertised only while the workspace
      // gate is on; the routes behind them refuse regardless.
      KIND_MEITNER_SHARED_COMPUTERS_ENABLED: sharedComputersEnabled(cfg) ? "1" : "0",
    },
  };
}
...
async function browserIntegration(botId: string, profile: string | undefined, turn?: { threadId: string; generation: string }) {
  ...
  return { profile: partitionId, session, spec, integration: {
    command: process.execPath, args: [SPAWNED_PROXIES.browser], env: {
      ...AGENTS_NODE_FLAG, KIND_MEITNER_BROWSER_TOKEN: token, KIND_MEITNER_HARNESS_URL: `http://127.0.0.1:${PORT}`,
    },
  } };
}
...
function phoneIntegration() {
  ...
  if (process.env.KIND_MEITNER_ADB_PATH) env.KIND_MEITNER_ADB_PATH = process.env.KIND_MEITNER_ADB_PATH;
  if (process.env.KIND_MEITNER_RESOURCES_PATH) env.KIND_MEITNER_RESOURCES_PATH = process.env.KIND_MEITNER_RESOURCES_PATH;
...
const computerControl = new ComputerControl((key, snapshot) => {
  ...
  if (snapshot.held && /^[A-Za-z0-9_-]{1,120}$/.test(botId)) {
    postDesktopPrivateMessage({ type: "kind-meitner:browser-control", botId, held: true });
  }
...
function previewSystemPrompt(bot: BotRecord) {
  ...
  const persona = [
    `You are ${bot.name}, a personal bot in kind-meitner.`,
  ...
  const coordination = bot.chiefOfStaff
    ? chiefOfStaffSystemPrompt(bot.id, store.bots, true, kindMeitnerStatusSystemPrompt())
    : peers.length > 0
...
const configuredSseHeartbeatMs = Number(process.env.KIND_MEITNER_SSE_HEARTBEAT_MS);
...
const TURN_STALL_MS = Math.max(60_000, Number(process.env.KIND_MEITNER_TURN_STALL_MS) || 20 * 60_000);
...
const ASK_BOT_TIMEOUT_MS = Math.max(5_000, Number(process.env.KIND_MEITNER_ASK_BOT_TIMEOUT_MS) || 4 * 60_000);
...
const GROUP_GOAL_WAIT_MAX_MS = Math.max(1_000, Number(process.env.KIND_MEITNER_GOAL_WAIT_MAX_MS) || 30 * 60_000);
...
bus.subscribe((event: RuntimeEvent) => {
  ...
  switch (event.type) {
    ...
    case "request.opened": {
      ...
      // kind-meitner decides nothing about the action itself. Only Full access
...
const EXTERNAL_CONTEXT_MARKER_PREFIX = "__kind-meitner_external_context__:";
...
async function startTurn(
  ...
  console.error(`[kind-meitner-turn] bot=${botId} text=${JSON.stringify(resolvedImages.text.slice(0, 70))} images=${turnImages.length} depth=${commsDepth} card=${Boolean(opts?.cardContinuation)}`);
  ...
  void (async () => {
    try {
      ...
      // user's work. Its private KindMeitner workspace is app-owned and changes
      ...
      if (wants === "vm") {
        ...
        if (!cua) throw new Error("CUA Driver is not ready for this computer — check permissions and restart kind-meitner");
        ...
        if (!unsupported) {
          ...
          if (remote?.ready && remote.sshAlias) {
            ...
            integrations.localComputer = {
              ...vpsMcp,
              env: { ...vpsMcp.env, KIND_MEITNER_CONTROL_URL: vpsControl.url, KIND_MEITNER_CONTROL_TOKEN: vpsControl.token },
            };
            computerKind = "vps";
...
const okxRecurringEngine = new OkxRecurringEngine({
  gateway: okxGateway,
...
store.reconcileInterruptedGroupGoals((runId, threadId) => {
  ...
  const detail = run.output ?? run.error ?? (
    status === "completed"
      ? "The scheduled team goal completed before kind-meitner restarted."
      : status === "stopped"
        ? "The scheduled team goal was stopped."
        : "kind-meitner restarted before this scheduled team goal finished."
  );
...
async function cloudRoutineReadiness(): Promise<{ ready: boolean; reason?: string }> {
  ...
  if (!instance) {
    return { ready: false, reason: "The Cloud VM runner is unavailable. Restart kind-meitner and try again." };
  }
  try {
...
async function deleteBotWithLifecycle(botId: string, revalidate: () => void = () => {}, setupRequest?: TeamSetupRequest) {
  const deletionResponse = (status: number, body: { error?: string; ok?: boolean }) => ({ status, body });
      ...
      try {
        ...
        // been renamed outside kind-meitner. The create journal is stronger
...
try {
  ...
  console.log(`kind-meitner webhook receiver on http://${webhookIngress.host}:${webhookIngress.port}${advertised}`);
} catch (error) {
  webhookIngressError = error instanceof Error ? error.message : String(error);
  console.error(`kind-meitner webhook receiver unavailable: ${webhookIngressError}`);
}
...
async function runGroupMemberTurn(
  ...
  const outsideRoom = integrations.agents
    ...
    : [];
  const system = [
    `You are ${bot.name}, a bot in the room "${readyGroup.name}" in kind-meitner.`,
    bot.title && `Role: ${bot.title}.`,
    bot.description && `About: ${bot.description}`,
...
function dispatchConnectorResume(entry: { botId: string; threadId: string; resumeKey: string; labels: string[] }) {
  ...
  const prompt = `kind-meitner connection update: the user securely connected ${names}. Continue the task that paused for this connection. Do not ask them to connect it again.`;
...
function credentialDesktopHandoff(label: string): string {
  return `Securely provide the ${label} from kind-meitner on your phone or computer. It is never added to chat.`;
}
...
function dispatchSecretResume(entry: SecretResumeEntry) {
  ...
  const prompt =
    entry.outcome === "provided"
      ? `kind-meitner credential update: the user securely provided ${entry.label}. Continue the task that paused for it. You do not receive the secret and must not ask them to paste it into chat.`
      : `kind-meitner credential update: the user declined to provide ${entry.label}. Continue without it if possible, or briefly explain the limitation. Do not ask them to paste it into chat.`;
  if (owner.group ? owner.bot.busy : threadBusy(entry.botId, entry.threadId) || activeGroupTurnForBot(entry.botId)) {
...
function cliProbeEnvironment(): NodeJS.ProcessEnv {
  ...
  for (const key of [
    ...
    "OPENCODE_API_KEY",
    "COMPOSIO_API_KEY",
    "KIND_MEITNER_COMPOSIO_BROKER_TOKEN",
    "KIND_MEITNER_TTS_KEY",
    "KIND_MEITNER_OPENAI_IMAGE_KEY",
    "KIND_MEITNER_CUSTOM_IMAGE_KEY",
    "ANTHROPIC_API_KEY",
    "OPENAI_API_KEY",
  ]) {
    delete env[key];
...
// ── HTTP plumbing ─────────────────────────────────────────────────────
/** The built UI, when this process serves it (KIND_MEITNER_STATIC_DIR: set by the
...
const handleRequest = async (req: IncomingMessage, res: ServerResponse) => {
  ...
  try {
    ...
    if (method === "GET" && path === "/api/health/hosted") {
      ...
      return json(res, 200, { ok: true, service: "kind-meitner", membershipAuthority: "portal", workspace: hosted.workspace, ...HOSTED_CONTRACT_METADATA });
    }
    ...
    if (method === "GET" && path === "/.well-known/kind-meitner/environment") {
    ...
    const domainCheck = /^\/\.well-known\/kind-meitner\/domain-check\/([a-f0-9]{64})$/.exec(path);
    ...
    if (method === "GET" && path === "/api/health" && !gate.auth) {
      return json(res, 200, { app: "kind-meitner" });
    }
    ...
    if (method === "POST" && path === "/api/auth/pairing") {
      ...
      return json(res, 200, {
        ...
        hint: base
          ? null
          : "this server has no public address to put in a link: set KIND_MEITNER_PUBLIC_URL, or open /pair on the address you use and type the code",
      });
    }
    ...
    if (path === "/api/settings/custom-domain") {
      ...
      if (method === "POST" || method === "DELETE") {
        if (DESKTOP_MANAGED) return json(res, 409, { error: "Custom domains are configured on a self-hosted kind-meitner server, not the desktop companion." });
        if (!/^application\/json\b/i.test(String(req.headers["content-type"] ?? ""))) {
    ...
    if (method === "POST" && path.startsWith("/api/shared-computers/") && sharedComputersEnabled(cfg)) {
      ...
      const secret = String(req.headers["x-kind-meitner-computer-secret"] ?? "");
    ...
    if (method === "POST" && path === "/api/testing/internal-capability") {
      const expected = process.env.KIND_MEITNER_TEST_INTERNAL_CAPABILITY_KEY ?? "";
      const actual = Array.isArray(req.headers["x-kind-meitner-test-capability"])
        ? ""
        : String(req.headers["x-kind-meitner-test-capability"] ?? "");
    ...
    // relay never has to expose the rest of kind-meitner's control surface.
    ...
    const requirePinnedClientThread = (botId: string, threadId: unknown): void => {
      if (threadId === undefined &&
        (auth.kind === "session" || req.headers["x-kind-meitner-companion"] === "1") &&
    ...
    // to kind-meitner's private attachment directory. This is deliberately not
    ...
    if (method === "POST" && path === "/api/teams/import") {
      ...
      if (body?.format === "kind-meitner.backup") {
    ...
    if (m && method === "POST") {
      ...
      if (auth.kind !== "loopback") {
        return json(res, 403, { error: `This only works on the computer running kind-meitner. The memory folder there is ${workspacePath}`, workspacePath });
      }
    ...
    if (method === "GET" && path === "/api/health") {
      return json(res, 200, { app: "kind-meitner", pid: process.pid, static: Boolean(STATIC_DIR) });
    }
    ...
    if (fleetRoute) {
      ...
      if (!fleetAvailable(socket)) return json(res, 404, { error: "No fleet agent on this server. Run `kind-meitner fleet init --domain … --operator <this user>` as root." });
    ...
    if (method === "POST" && instanceAction) {
      ...
      try {
        ...
        if (action === "install") {
          if (!(await registry.installRuntime(instanceId))) return json(res, 404, { error: "Installing this engine from Settings is not available on this server. Use the install command on the machine running kind-meitner." });
          return json(res, 200, { instances: await describeInstances() });
        }
    ...
    if (m && method === "POST") {
      if (req.headers["x-kind-meitner-companion"] !== "1") {
        return json(res, 403, { error: "Secure phone entry must come from a paired phone" });
      }
      const rawDeviceId = req.headers["x-kind-meitner-companion-device"];
...
// A second listener for `kind-meitner serve --tunnel` (server/tunnel.ts): the
...
const TUNNEL_SOCKET = process.env.KIND_MEITNER_TUNNEL_SOCKET?.trim() || null;
...
if (TUNNEL_SOCKET) {
  ...
  tunnelListener.listen(TUNNEL_SOCKET, () => {
    console.log(`kind-meitner tunnel listener on ${TUNNEL_SOCKET}`);
  });
}
